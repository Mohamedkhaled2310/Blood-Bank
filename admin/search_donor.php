...
<table border='1' id='table' dir="rtl">
            ...
            $search = "SELECT * FROM poll WHERE t_blood LIKE '%$filter%' or name LIKE '%$filter%'  limit $page1,10";
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                echo "<td><input type = 'text' name = 'feedback' value = '". $row['feedback'] ."'><input type = 'hidden' name = 'id' value = '". $row['id'] ."'><button type = 'submit' name = 'submit5'>حفظ</button></td>";
                echo "</form>";
                echo "</tr>";
            }
     ?>
        ...
        for($i = 1;$i<= $num_page;$i++){
         ?><a href = "search_donor.php?page=<?php echo $i;?>&filter=<?php echo $filter;?>" style = "text-decoration:none;border-raduis:10%;padding:10px;background-color:#0c1736;color:aliceblue;"><?php echo $i." "; ?></a><?php
        }
...
if(isset($_SESSION['status']) && $_SESSION['status'] != '')
{
    ...
    <script>
        swal({
            ...
            .then((confirm) => {
                if (confirm)
                    window.location = "search_donor.php?filter=<?php echo $filter2;?>";
